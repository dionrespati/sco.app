...
class Userconfig_service extends MY_Service {
	...
	function changePassword($data) {
		$userResult = $this->m_userconfig->getListAllUserByParam("username", $data['username']);	
		if($userResult === null) {
			return jsonFalseResponse("Username tidak terdaftar di dalam sistem");
		}   

		if($userResult[0]->password !== $data['old_password']) {
			return jsonFalseResponse("Password lama tidak sesuai..");
		}	

		$savePwd = $this->m_userconfig->updatePassword($data);
		return $savePwd;
	}
}