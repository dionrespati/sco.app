...
<table width="100%" class="table table-striped table-bordered bootstrap-datatable datatable" align="center">
  <tr>
    <th colspan="5">List SSR / MSR Testing DION</th>
  </tr>
  <tr>
    <th>SSR/MSR No</th>
    <th>Stockist</th>
  ...
  foreach($listSSR as $ssr) {
    ...
    $third_prefix = substr($ssr->batchno, 0, 3);
    ...
    if($third_prefix === "PVR") {
      echo "<input type='button' class='btn btn-mini btn-warning' value=\"Approve PVR\" onclick=\"approvePVR('$ssr->batchno','$ssr->sc_dfno')\" />&nbsp;";
    }
    echo "<a class='btn btn-mini btn-primary' onclick=\"$urlviewInv\">Proses</a>";
    echo "</td>";
    echo "</tr>";
  }
  ?>
...
function approvePVR(nossr, kodestk) {
  All.set_disable_button();
		$.ajax({
      url: All.get_url('bo/cnmsn/pvr/approve'),
      type: 'POST',
      dataType: 'json',
      data: {batchno: nossr, dfno: kodestk},
      success:
      function(data){
