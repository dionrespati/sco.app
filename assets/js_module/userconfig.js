...
var Userconfig = {
	...
	checkNewUsername : function(paramValue) {
		if(paramValue !== "") {
	    	All.set_disable_button();
			All.get_wait_message();
			$.ajax({
	            url: All.get_url("user/list/username/" +paramValue),
	            type: 'GET',
				dataType: 'json',
	            success:
	            function(data){
	                ...
	                if(data.response == "true") {
						$.ajax({
							url: All.get_url("user/id/" +paramValue) ,
							type: 'GET',
							dataType: 'json',
							success:
							function(data){
								...
								if(data.response == "true") {
									...
									$(All.get_active_tab() + " #departmentid").val(data.arrayData[0].departmentid);
									$(All.get_active_tab() + " #groupid").val(data.arrayData[0].groupid);
									$(All.get_active_tab() + " #prev_period_bnsmonth").val(data.arrayData[0].prev_period_bnsmonth);
