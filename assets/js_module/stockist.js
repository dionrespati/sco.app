...
var Stockist = {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.response == "true") {
                    ...
                    if (data.arrayData[0].kode_provinsi != null) {
                        $(All.get_active_tab() + " #province").val(data.arrayData[0].kode_provinsi);
                    } else {
                        $(All.get_active_tab() + " #province").val(null);
                    }

                    if (data.arrayData[0].kode_kabupaten != null) {
                        $(All.get_active_tab() + " #kabupaten").val(data.arrayData[0].kode_kabupaten);
                    } else {
                        $(All.get_active_tab() + " #kabupaten").val(null);
                    }

                    if (data.arrayData[0].kode_kecamatan != null) {
                        $(All.get_active_tab() + " #kecamatan").val(data.arrayData[0].kode_kecamatan);
                    } else {
                        $(All.get_active_tab() + " #kecamatan").val(null);
                    }
                } else {
                    alert(data.message);
                    All.reset_all_input();
    ...
    getProductPriceKhusus: function (param) {
        var prdcd = $(All.get_active_tab() + " #prdcd" + param).val();
        var pricecode = $(All.get_active_tab() + " #pricecode").val();
        var prd_voucher = $(All.get_active_tab() + " #prd_voucher").val();
        var jenis_trx = $(All.get_active_tab() + " #jenis_bayar").val();
        var tipe_vchpromo = $(All.get_active_tab() + " #tipe_vchpromo").val();
        All.set_disable_button();
        /* $.ajax({
            url: All.get_url("product/id/" +prdcd+ "/" +pricecode) ,
            type: 'GET',
        ...
        $.ajax({
            dataType: 'json',
            url: All.get_url("sales/vcashKhusus/prdCheck"),
            type: 'POST',
            data: {
                productcode: prdcd,
                pricecode: pricecode,
                jenis: jenis_trx,
                tipe_vchpromo: tipe_vchpromo
            },
            success: function (data) {
                All.set_enable_button();
                if (data.response == "true") {
    ...
    calculateProductVchKhusus: function (param) {
        var qty = $(All.get_active_tab() + " #jum" + param).val();
        var nm = /^[0-9]+$/;
        if (qty == '0' || qty == '' || !qty.match(nm)) {
            $(All.get_active_tab() + " #jum" + param).val(1);
            $(All.get_active_tab() + " #jum" + param).select();
            Stockist.calculateAllPriceVchCash();
    ...
    calculateAllPriceVchCash: function () {
    ...
    addNewRecordPrd: function () {
        ...
        j++;
        rowhtml += "<td><input attr='jqty' onkeyup=Stockist.calculateProduct(" + z + ") tabindex=" + j + " style='text-align:right;' type='text' class='span12 typeahead jumlah' id=jum" + z + "  name=jum[] value='' /></td>";
    ...
    addNewRecordPrdVchCash: function () {
        var rec_awal = $(All.get_active_tab() + " #amount").val();
        var amount = parseInt(rec_awal);
        var tabidx = parseInt($(All.get_active_tab() + " #tabidx").val());
        ...
        rowhtml += "<td><input readonly=readonly style='text-align:right' type='text' class='span12 typeahead jumlah' id=harga" + z + "  name=harga[] value='' /></td>";
        rowhtml += "<td><input readonly=readonly style='text-align:right' type='text' class='span12 typeahead' id=sub_tot_bv" + z + "  name=sub_tot_bv[] value='' /></td>";
        rowhtml += "<td><input readonly=readonly style='text-align:right' type='text' class='span12 typeahead' id=sub_tot_dp" + z + "  name=sub_tot_dp[] value='' /><input style='text-align:right' type='hidden' class='span12 typeahead' id=sub_tot_dp_real" + z + " attr=prd  name=sub_tot_dp_real[] value='' /></td>";
        rowhtml += "<td align=center><a onclick=javascript:Stockist.delPayment(this) class='btn btn-mini btn-danger'><i class='icon-trash icon-white'></i></a></td>";
        rowhtml += "</tr>";
        var y = j + 1;
        //console.log("next tab idx : " +y);
    ...
    addNewRecordPrdVchCashKhusus: function () {
        ...
        var rowhtml = "<tr>";
        rowhtml += "<td><input onchange=Stockist.getProductPriceKhusus(" + z + ") tabindex=" + j + " type='text' class='span12 typeahead' id=prdcd" + z + "  name=prdcd[] value=''/></td>";
        rowhtml += "<td><input readonly=readonly type='text' class='span12 typeahead' id=prdnm" + z + "  name=prdnm[] value=''/></td>";
        j++;
        rowhtml += "<td><input onkeyup=Stockist.calculateProduct(" + z + ") tabindex=" + j + " style='text-align:right;' type='text' class='span12 typeahead jumlah' id=jum" + z + "  name=jum[] value='' /></td>";
        rowhtml += "<td><input readonly=readonly style='text-align:right' type='text' class='span12 typeahead jumlah' id=poin" + z + "  name=poin[] value='' /></td>";
        ...
        var new_rec = amount + 1;
        $(All.get_active_tab() + " #amount").val(new_rec);
        console.log("amount : " + new_rec);
        $(All.get_active_tab() + " #addPrd").append(rowhtml);
        $(All.get_active_tab() + " #addRow").removeAttr("tabindex");
        $(All.get_active_tab() + " #addRow").attr("tabindex", y);
        console.log("rec : " + new_rec);
                            ...
                            if (data.response == "true") {
                                ...
                                if (detProd != null) {
                                    ...
                                    $.each(detProd, function (key, value) {
                                        ...
                                        rowPrd += "<td><input id=sub_tot_dp" + prdx + " readonly=readonly style='text-align:right' type='text' class='span12 typeahead' name=sub_tot_dp[] value='" + All.num(parseInt(value.qtyord * value.dp)) + "' />";
                                        rowPrd += "<input id=sub_tot_dp_real" + prdx + " readonly=readonly style='text-align:right' type='text' class='span12 typeahead' attr=prd name=sub_tot_dp_real[] value='" + parseInt(value.qtyord * value.dp) + "' />";
                                        rowPrd += "</td>";
    ...
    saveTrxDeposit: function (url, formid) {
        All.set_disable_button();
    ...
    recalculateDeposit: function (id_deposit) {
        url = 'scan/deposit/recalculate/' + id_deposit;
        All.set_disable_button();
        //All.get_image_load();
        $.ajax({
            url: All.get_url(url),
    ...
    koreksiTransaksi: function (trcd) {
        url = 'sales/correction/' + trcd;
        All.set_disable_button();
        //All.get_image_load();
        $.ajax({
            url: All.get_url(url),
        ...
        $.post(All.get_url(url), $(All.get_active_tab() + "#" + formid).serialize(), function (data) {
            ...
            if (data.response == "true") {
                ...
                if (res.pref_trcd == "PV") {
                    ...
                    urlx = "sales/sub/input/formV2" + "/" + res.sc_dfno + "/" + res.sc_co;
                    tipeTtp = "ttp";
                }
                rowhtml += "<tr>";
                if (tipeTtp == "pv") {
    ...
    get_vch_info: function (tipe) {
        ...
        if (distributorcode == '0' || distributorcode == '' || vchno == '0' || vchno == '') {
            ...
            $.ajax({
                type: 'POST',
                url: All.get_url("sales/vcashKhusus/check"),
                dataType: 'json',
                data: {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.response == 'false') {
                        ...
                        var tipe_promo = 0;
                        var jum_vch = 0;
                        if (vchdpn == "XPV" || vchdpn == "ZVO" || vchdpn == "XPP" || vchdpn == "XHD" || vchdpn == "AYU") {
    ...
    helper_show_prod_by_id: function (frm) {
        ...
        var jenis_trx = $(All.get_active_tab() + " #jenis_bayar").val();
        console.log("jenis trx : " + jenis_trx);
        var counter = 0;
    ...
    count_qty_product_vcash: function (param) {
        var qty = $(All.get_active_tab() + " #qty" + param).val();
        var price = $(All.get_active_tab() + " #dp_real" + param).val();
        sub_total = price * qty;
        $(All.get_active_tab() + " #total_dp" + param).val(All.num(sub_total));
    ...
    hapusTTPvchDeposit: function (notrx) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.response == "true") {
                    $(All.get_active_tab() + " .nextForm2").html(null);
                    var urlx = 'scan/list/detail/ttp/' + id_deposit;
                    $(All.get_active_tab() + " .nextForm1").css("display", "block");
                    All.ajaxShowDetailonNextForm(urlx);
                }
            },
            error: function (xhr, ajaxOptions, thrownError) {
                alert(thrownError + ':' + xhr.status);
