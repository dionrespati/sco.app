...
class CI_Jquery_pagination{
   ...
   var $param_prd     = '';
			...
			if ($i >= 0)
			{
				if ($this->cur_page == $loop)
				{
					...
					$output .= $this->num_tag_open
						. $this->getAJAXlink( $this->cur_page, $loop )
						. $this->num_tag_close;
				}
				else
				{
	...
	function getAJAXlink( $count, $text) {
		...
		/*return "<a href=\"#\"
		         " . $this->anchor_class . "
					onclick=\"$.post('". $this->bae_url . $count ."', ". $this->additional_param .", function(data){
					$('". $this->div . "').html(data);" . $this->js_rebind ."; }); return false;\">"
				. $text .'</a>'; */
		return "<a id=\"".$this->param_prd."\" rel=\"".$text."\" href=\"#\" onclick=\"Shopping.getProdByCat2(this)\">". $text ."</a>";
		//$this->cur_page = (int) $text;		
	}
